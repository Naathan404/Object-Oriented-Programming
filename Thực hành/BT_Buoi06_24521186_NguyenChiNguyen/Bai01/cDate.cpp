...
int cDate::SoSanh(cDate d) const
{
	if (this->iNam < d.iNam)
		return 1;
	else if (this->iNam > d.iNam)
		return -1;
	else
	{
		if (this->iThang < d.iThang)
			return 1;
		else if (this->iThang > d.iThang)
			return -1;
		else
		{
			if (this->iNgay < d.iNgay)
				return 1;
			else if (this->iNgay > d.iNgay)
				return -1;
			else
				return 0;
		}
