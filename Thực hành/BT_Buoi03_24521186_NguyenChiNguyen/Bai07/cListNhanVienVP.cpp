...
cListNhanVienVP::cListNhanVienVP(int size)
{
	List.resize(size);
}
...
cNhanVienVP cListNhanVienVP::TimNVLuongCaoNhat() const
{
	cNhanVienVP KQ;
	if (List.size() == 0)
	{
	...
	KQ = List[0];
	float luongMax = List[0].getLuong();
	for (int i = 1; i < List.size(); i++)
	{
		if (List[i].getLuong() > luongMax)
		{
			luongMax = List[i].getLuong();
			KQ = List[i];
		}
	}
	return KQ;
...
int cListNhanVienVP::TinhTongLuong() const
{
	int tongLuong = 0;
	for (int i = 0; i < List.size(); i++)
	{
...
cNhanVienVP cListNhanVienVP::TimNVTuoiCaoNhat() const
{
	cNhanVienVP KQ;
	if (List.size() == 0)
	{
...
// Cai dat quicksort
int Partition(vector<cNhanVienVP>& ds, int l, int r, bool tang)
{
	cNhanVienVP pivot = ds[(l + r) / 2];
	int i = l, j = r;
	while (i <= j)
	{
		if (tang)
...
void QuickSort(vector<cNhanVienVP>& ds, int l, int r, bool tang)
{
	if (l >= r)
		return;
...
void cListNhanVienVP::SapXepTheoLuong(bool SXTang)
{
